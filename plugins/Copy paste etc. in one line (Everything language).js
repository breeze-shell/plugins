...
shell.menu_controller.add_menu_listener(ctx => {
    try {
        ...
        const small_buttons = [
            {
                icon: 'copy',
                orig_resid: '29713@SHELL32.dll',
                orig_name: ['复制', 'Copy']
            }, {
                icon: 'cut',
                orig_resid: '31260@SHELL32.dll',
                orig_name: ['剪切', 'Cut']
            }, {
                icon: 'delete',
                orig_resid: '33246@SHELL32.dll',
                orig_name: ['删除', 'Delete']
            }, {
                icon: 'paste',
                orig_resid: "31396@SHELL32.dll",
                orig_name: ['粘贴']
            },
            {
                icon: 'rename',
                orig_resid: "31258@SHELL32.dll",
        ...
        // Проверяем, есть ли name_resid и соответствует ли оно button.orig_resid
        if (data.name_resid === button.orig_resid) {
            return true;
        }

        // Если name_resid не совпадает или отсутствует, возвращаем false
        return false;
    })

    // Теперь получаем только data.name_resid из corresponding_item
    if (corresponding_item) {
        const data = corresponding_item.data();
        shell.println(data.name_resid);
    }
